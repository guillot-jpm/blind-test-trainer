...
def test_add_duplicate_song_spotify_id(db_connection):
    """Test that adding a song with a duplicate spotify_id raises an error."""
    song_library.add_song("Song 1", "Artist A", 2021, "song1.mp3", spotify_id="unique-id-1")
    with pytest.raises(song_library.DuplicateSongError):
        song_library.add_song("Song 2", "Artist B", 2022, "song2.mp3", spotify_id="unique-id-1")
